...
BOOL HandlePrivateMessage(ClientsContainer *clients, Client *client, ParsedMessage *parsed_message);

// Handles a broadcast message
BOOL HandleBroadcastMessage(ClientsContainer *clients, Client *client, ParsedMessage *parsed_message);

// Handles an active users command
BOOL HandleActiveUsersCommand(ClientsContainer *clients, Client *client);

BOOL SendMessageToSenderThread(
	ClientsContainer *clients, 
	Client *client, 
	char *dest_user,
	unsigned int dest_user_length,
	char *message_body,
	BOOL is_broadcast
);
...
BOOL HandleMessage(ClientsContainer *clients, Client *client, ParsedMessage *parsed_message)
{
	...
	case REQ_BROADCAST_MESSAGE:
		break;
	case REQ_ACTIVE_USERS:
		break;
	case REQ_BROADCAST_FILE:
	case REQ_P2P_MESSAGE:
		LOG_DEBUG("Unhandled message (optinal request type - %d)", req_type);
		break;
	case REQ_QUIT_MESSAGE:
		...
		break;
	default:
		LOG_ERROR("Unknown request type (%d) in message to handle", req_type);
		goto cleanup;
	}
...
BOOL HandlePrivateMessage(ClientsContainer *clients, Client *client, ParsedMessage *parsed_message)
{
	...
	LOG_WARN("Message handling temporarily not implemented");
	result = TRUE;
...
BOOL HandleQuitCommand(ClientsContainer *clients, Client *client)
{
	...
	if (!SendMessageToSenderThread(
		...
		TRUE))
	{
		LOG_ERROR("Failed to send message to sender thread");
		goto cleanup;
	}
...
BOOL SendMessageToSenderThread(
	...
	MsgToSend *msg_to_send = NULL;
	ChatMessage *chat_message = NULL;
	BOOL result = FALSE;
	
	LOG_ENTER_FUNCTION();

	msg_to_send = (MsgToSend *)malloc(sizeof(*msg_to_send));
	if (msg_to_send == NULL)
	{
		LOG_ERROR("Failed to allocate memory");
		goto cleanup;
	...
	if (!BuildChatMessage(
		...
		SYSTEM_MESSAGE,
		message_body,
		strlen(message_body)+1)
	) {
		LOG_ERROR("Failed to build the chat response");
		goto cleanup;
	}
	
	// Set the message sender and dest
	if (is_broadcast)
	{
		if (memcpy(msg_to_send->sender, client->username, client->username_length) == NULL)
		{
			LOG_ERROR("Failed to copy string");
			goto cleanup;
		}
		msg_to_send->sender_length = client->username_length;
		// Don't set the dest username since it is a broadcast message
		msg_to_send->dest_length = 0;

	} else {
		if (memcpy(msg_to_send->sender, SERVER_USERNAME, strlen(SERVER_USERNAME)) == NULL)
		{
			LOG_ERROR("Failed to copy string");
			goto cleanup;
		}
		msg_to_send->sender_length = strlen(SERVER_USERNAME);

		if (dest_user == NULL)
		{
			LOG_ERROR("Received NULL dest_user for non-broadcast message");
			goto cleanup;
		}
		if (memcpy(msg_to_send->dest, dest_user, dest_user_length) == NULL)
		{
			LOG_ERROR("Failed to copy string");
			goto cleanup;
		}
		msg_to_send->dest_length = dest_user_length;
	}

	msg_to_send->is_broadcast = is_broadcast;
	msg_to_send->message = chat_message;

	if (!Enqueue(clients->send_queue, (void *)msg_to_send))
	{
		LOG_ERROR("Failed to send the message");
		goto cleanup;
	}
	LOG_DEBUG("Message enqueued");
